...
void RTCCI2C::setMonth(uint8_t dest, uint8_t value)
{
    ...
    if( value < 0x13 && value > 0)
    {
        ...
        //set month
        Wire.beginTransmission(0x6F); 
        Wire.send(rgbMonth[0]);   
        Wire.send(rgbMonth[1]);    
        Wire.endTransmission(); 
	}
}
/* ------------------------------------------------------------------- */
